    ...
    for fold in folds:
        if fold == teste:
            for participante in folds[fold].keys():
                testeSet.append(folds[fold][participante])
        else:
            for participante in folds[fold].keys():
                treinamentoSet.append(folds[fold][participante])
...
def getListaPreferencia(frequencia, atributos):
    ...
    for participante in frequencia.keys():
        ...
        if "next-to" in frequencia[participante]["target"].keys():
            frequenciaRelacionais["next"] = frequencia[participante]["target"]["next"]
            frequenciaRel = frequenciaRel + frequencia[participante]["target"]["next"]
            del(frequencia[participante]["target"]["next"])
