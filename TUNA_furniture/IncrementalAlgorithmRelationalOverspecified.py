...
class IncrementalAlgorithmRelationalOverspecified(object):
    def __init__(self, dominio = {}, targets = [], preferred_attributes = [], frequencia = {}, probabilidade = float, tamanho = int, restricaoContexto = False):
    ...
    def run(self):
        for target in self.targets:
            self.distractors[target] = self.initializeDistractors(target, self.restricaoContexto, self.dominio)
            self.atributos[target] = self.listaAtributos(target, True)
            self.description[target] = {}
            self.description = self.searchDescription(target)
    ...
    def checaProbabilidade(self, atributo, objeto):
        if objeto in self.targets:
            if self.relationalAttribute(atributo):
                ...
                if atributo in self.frequencia["target"].keys():
            ...
            if atributo in self.frequencia.keys():
