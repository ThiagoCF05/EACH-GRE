...
def test(svms, teste):
    ...
    for anotacao in teste:
        ...
        for svm1 in svms:
            ...
            if svm1 in ["lm_type", "lm_colour", "lm_vpos", "lm_hpos"] and (anotacao["classes"]["above"] > 0 or anotacao["classes"]["below"] > 0 or anotacao["classes"]["near"] > 0 or anotacao["classes"]["right"] > 0 or anotacao["classes"]["left"] > 0):
                if real == resultado[0]:
                ...
                if real == 0 and resultado[0] == 0:
                    ...
                    fp[svm1] = fp[svm1] + 1
            elif svm1 in ["lm2_type", "lm2_colour", "lm2_vpos", "lm2_hpos"] and (anotacao["classes"]["lm_above"] > 0 or anotacao["classes"]["lm_below"] > 0 or anotacao["classes"]["lm_near"] > 0 or anotacao["classes"]["lm_right"] > 0 or anotacao["classes"]["lm_left"] > 0):
                if real == resultado[0]:
                    acertos[svm1] = acertos[svm1] + 1.0
...
def trainRelationalAttributes(svms, treinamento, CI, gammaI, combinacoes):
    for svm1 in svms:
        ...
        for fold in treinamento:
            for anotacao in treinamento[fold]:
                if anotacao["classes"]["lm_above"] > 0 or anotacao["classes"]["lm_below"] > 0 or anotacao["classes"]["lm_near"] > 0 or anotacao["classes"]["lm_right"] > 0 or anotacao["classes"]["lm_left"] > 0:
                    data.append(anotacao["data"])
                    classe.append(anotacao["classes"][svm1])
